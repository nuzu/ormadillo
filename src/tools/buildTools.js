async function dropAllTables() {
    ...
    if(!tools) return console.log("Unable to build because invalid dialect access is being attempted")
    return await tools.dropAllTables.call(this)
}

async function hasTable(tableName) {
    const tools = this._tools
    if(!tools) return console.log("No tools")
...
async function createTables(mappers) {
    ...
    mappers = await Promise.all(mappers.map(async mapper => {
        ...
        if(!response) await createTable.call(this, mapper)
        return mapper
    }))
...
async function createTable(mapper) {
    const tools = this._tools
    if(!tools) return console.log("Unable to build because invalid dialect access is being attempted") 
    if(!mapper.name) throw "Each model needs to have a name"
